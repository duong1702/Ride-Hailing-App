...
import 'package:taxi_users_app/widgets/Cancel_trips.dart';
...
class TruckBookingPage extends StatefulWidget {
  const TruckBookingPage({super.key});

  @override
  State<TruckBookingPage> createState() => _TruckBookingPage();
}

class _TruckBookingPage extends State<TruckBookingPage> {
  ...
  double truckDetailsContainerHeight = 0;
  ...
  String selectedTruckType = "small";
  double cargoWeight = 100.0;
  String selectedVehicleType = '';
  OnlineNearbyDrivers? currentDriver; // Thông tin tài xế
  ...
  displayUserRideDetailsContainer() async {
    ...
    setState(() {
      ...
      truckDetailsContainerHeight = 280;
          ...
          case Geofire.onKeyEntered:
            ...
            driverRef.once().then((DatabaseEvent dataSnapshot) {
              ...
              if (dataSnapshot.snapshot.value != null) {
                ...
                onlineNearbyDrivers.vehicleType = driverData["vehicleType"];  // Lấy vehicleType từ Firebase
                print("Driver Added: ${onlineNearbyDrivers.uidDriver}, Vehicle Type: ${onlineNearbyDrivers.vehicleType}");
              } else {
  ...
  void makeTripRequest(String vehicleType) {
    if (vehicleType.isEmpty) {
      ...
      cMethods.displaySnackBar("Vui lòng chọn loại xe trước khi đặt chuyến", context);
      return;
    }

    // Tạo tham chiếu mới trong Firebase
  ...
  updateFromDriverCurrentLocationToDropOffDestination(
    ...
    if (!requestingDirectionDetailsInfo) {
      ...
      setState(() {
        tripStatusDisplay =
        "Tài xế đang trong chuyến đi - ${directionDetailsPickup.durationTextString}";
      });
  ...
  void searchDriver() {
    ...
    var filteredDrivers = availableNearbyOnlineDriversList!.where((driver) {
      ...
      return driver.vehicleType == selectedVehicleType;  // Lọc tài xế theo vehicleType
    }).toList();
  ...
  sendNotificationToDriver(OnlineNearbyDrivers currentDriver) {
    ...
    tokenOfCurrentDriverRef.once().then((dataSnapshot) {
      ...
      var timerCountDown = Timer.periodic(oneTickPerSec, (timer) {
        ...
        // Kiểm tra nếu người dùng hủy chuyến đi
        if (stateOfApp != "requesting") {
          timer.cancel();
          currentDriverRef.set("cancelled"); // Cập nhật trạng thái chuyến đi là đã hủy
          currentDriverRef.onDisconnect();
          requestTimeoutDriver = 20;

          // Gửi thông báo cho tài xế về việc hủy chuyến đi
          sendCancellationNotificationToDriver(currentDriver);
        }
  ...
  // Hàm gửi thông báo hủy chuyến cho tài xế
  sendCancellationNotificationToDriver(OnlineNearbyDrivers currentDriver) {
    // Lấy mã thông báo thiết bị của tài xế
    DatabaseReference tokenOfCurrentDriverRef = FirebaseDatabase.instance
        .ref()
        .child("drivers")
        .child(currentDriver.uidDriver.toString())
    ...
    tokenOfCurrentDriverRef.once().then((dataSnapshot) {
      if (dataSnapshot.snapshot.value != null) {
        ...
        // Gửi thông báo hủy chuyến cho tài xế
        PushNotificationService.sendNotificationToSelectedDriver(
            deviceToken, context, "Chuyến đi đã bị hủy bởi người dùng.");
      } else {
        return;
      }
    });
      ...
      body: Stack(
        children: [
          ...
          Positioned(
            ...
            bottom: 0,
            child: Container(
              height: truckDetailsContainerHeight,
              decoration: const BoxDecoration(
                color: Colors.white,
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        Row(
                          children: [
                            Image.asset(
                              "assets/images/delivery.png", // Icon xe tiêu chuẩn
                              height: 40,
                              width: 70,
                            ),
                    ...
                    // Trọng lượng hàng hóa (Cố định 500kg)
                    const Text(
                      'Trọng lượng hàng: 300 kg',
                      style: TextStyle(
                        fontSize: 14,
                        color: Colors.black87,
                      ),
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Text(
                          tripDirectionDetailsInfo != null
                              ? "${cMethods.calculateTruckFareAmount(tripDirectionDetailsInfo!).toString()} VND"
                              : "0 VND",
                          style: const TextStyle(
                            fontSize: 16,
                            fontWeight: FontWeight.bold,
